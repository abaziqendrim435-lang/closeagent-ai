...
Premium dark-mode Streamlit command center: Apify → OpenAI.
Hunter.io email verification is temporarily disabled.
"""
...
try:
    from pipeline import (
        ...
        # enrich_leads_with_hunter,  # Hunter.io temporarily disabled
        # extract_lead_candidates,  # Hunter.io temporarily disabled
...
SIMULATION_STEPS = [
    ...
    # ("Verifying emails with Hunter.io...", 0.2),  # Hunter.io temporarily disabled
...
def run_agent(
    ...
    """Apify scrape → OpenAI drafts (Hunter.io email verify temporarily disabled)."""
    # hunter_key is unused while Hunter.io is commented out below.
    ...
    try:
        ...
        # Hunter.io temporarily disabled — send Apify Title / URL / Snippet to OpenAI.
        # candidates = extract_lead_candidates(organic)
        leads = targets_from_apify(organic)
    ...
    # --- Hunter.io email find/verify (temporarily disabled) ---
    # placeholder.markdown(
...
def _csv_bytes(leads: list[dict[str, str]]) -> bytes:
    ...
    writer = csv.DictWriter(
        buffer,
        fieldnames=["title", "url", "snippet", "company", "pain_point", "draft"],
    )
    writer.writeheader()
    for lead in leads:
        writer.writerow(
...
""",
    ...
    with st.form("mission_control"):
        ...
        # Hunter.io temporarily disabled — key is not required for this run.
    ...
    # hunter_key = hunter_input.strip() or saved_hunter
    hunter_key = saved_hunter  # Hunter.io temporarily disabled; not sent to the pipeline
    ...
    if unleash:
        if not apify_token or not target.strip():
            ...
            # Hunter.io key check temporarily disabled:
            # missing = []
            # if not openai_key:
            #     missing.append("OpenAI API key")
            # if not hunter_key:
            #     missing.append("Hunter.io API key")
    ...
    if st.session_state.show_results and st.session_state.leads:
        ...
        st.success(
            f"Generated {len(st.session_state.leads)} cold emails from Apify "
            "Title / URL / Snippet data (Hunter.io skipped). Drafts written with OpenAI."
        )
        st.download_button(
            "Download emails CSV",
        ...
        # render_leads_table(st.session_state.leads)  # Hunter.io table temporarily disabled
