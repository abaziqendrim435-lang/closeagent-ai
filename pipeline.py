"""CloseAgent SDR pipeline: Apify search → OpenAI drafts.

Hunter.io email find/verify is temporarily disabled; Apify Title / URL / Snippet
rows go straight to OpenAI.
"""
...
def targets_from_apify(
    ...
    """Map Apify Title / URL / Snippet rows into OpenAI draft targets.

    Hunter.io enrichment is skipped: each SERP row becomes one cold-email target.
    """
...
These are REAL companies discovered via web search (Apify). You only have three facts
per target: the page Title, the URL, and the search Snippet. Do not invent people,
emails, or companies that are not implied by those facts.

{leads_block}

For EACH target company:
1. Infer the company name and a sharp, realistic pain point (1 sentence) from Title + URL + Snippet.
2. Write a hyper-personalized, attention-grabbing cold sales email.
...
{{
  "leads": [
    {{
      "name": "company or page name inferred from Title",
...
def draft_emails_with_openai(
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Write hyper-personalized cold emails from Title, URL, and Snippet only. "
                    "Always respond with valid JSON matching the requested schema. "
                    "Never invent new companies."
                ),
            },
            {
                "role": "user",
    ...
    for index, lead in enumerate(leads):
        ...
        draft = str(item.get("draft") or item.get("email") or "").strip()
